...
If Bouncie is not connected (no OAuth token) and the admin asks how to connect, why mileage sync isn't working, or anything about "Bouncie not configured":
- Tell them to open **System Settings** in the admin dashboard.
- Scroll to the **📡 GPS Tracking (Bouncie)** section — it will show ❌ Not Connected.
- Click the **"Connect Bouncie"** button. A new tab opens and redirects to Bouncie's OAuth page.
- After authorizing, they are redirected back automatically and the token is saved. Mileage sync activates within 5 minutes.
- If they are already connected and need to re-authorize, the same section shows a **"Reconnect Bouncie"** button.
...
- If get_mileage returns bouncie_configured: false, explain that the Bouncie GPS integration is not yet connected. Tell the admin to go to **System Settings → GPS Tracking (Bouncie)** in the admin dashboard and click the **"Connect Bouncie"** button to start the OAuth flow. Once authorized, the token is saved automatically and mileage sync activates within 5 minutes.
