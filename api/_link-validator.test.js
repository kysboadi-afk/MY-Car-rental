...
test("BASE_URL is https://slycarrentals.com", () => {
  assert.equal(BASE_URL, "https://slycarrentals.com");
});
...
test("validateLink: returns ok=true when HEAD returns 200", async () => {
  ...
  const r = await validateLink("https://slycarrentals.com/balance.html");
  assert.equal(r.ok,           true);
  assert.equal(r.status,       200);
  assert.equal(r.fallbackUsed, false);
  assert.equal(r.url, "https://slycarrentals.com/balance.html");
});

test("validateLink: url is preserved in result when page is reachable", async () => {
  setFetch(async () => fakeResp(200));
  const original = "https://slycarrentals.com/balance.html?ext=1&cs=secret";
...
test("validateLink: returns ok=false when HEAD returns 404", async () => {
  ...
  const r = await validateLink("https://slycarrentals.com/missing.html");
...
test("validateLink: validates baseUrlForValidation but returns original url on success", async () => {
  ...
  const fullLink = "https://slycarrentals.com/balance.html?cs=secret123";
...
test("validateLink: returns custom fallback when base page is unreachable", async () => {
  ...
  const r = await validateLink("https://slycarrentals.com/balance.html?cs=s", {
    baseUrlForValidation: PAGE_URLS.balance,
...
test("validateLink: custom fallback is used when provided", async () => {
  setFetch(async () => fakeResp(404));
  const r = await validateLink("https://slycarrentals.com/balance.html", {
    fallback: "https://slycarrentals.com/manage-booking.html",
  });
  assert.equal(r.url, "https://slycarrentals.com/manage-booking.html");
});
