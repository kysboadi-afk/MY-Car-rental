...
//   Enter the same key in the Bouncie Developer Portal when registering the webhook.
//   Bouncie sends the key in Authorization and X-Bouncie-Authorization headers.
//
// Handled event types:
//   tripEnd     — most reliable odometer source → update mileage + insert trip_log
//   tripMetrics — driving behaviour stats       → insert trip_log (distance, speed)
...
//
// Required env vars:
//   BOUNCIE_WEBHOOK_SECRET            — must match the key set in Bouncie Developer Portal
//   SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY

import { loadTrackedVehicles, updateVehicleMileage, insertTripLog } from "./_bouncie.js";
...
export default async function handler(req, res) {
  ...
  if (!sb) {
    // Accept 200 so Bouncie does not retry when Supabase is not configured
  ...
  try {
    switch (eventType) {
      ...
      // ── Trip end: most reliable odometer reading ────────────────────────
      case "tripEnd": {
      ...
      case "mil": {
        ...
        if (mil?.value === "ON") {
          ...
          }).catch((err) => console.warn("bouncie-webhook: mil log failed:", err.message));
        }
        break;
      }
      ...
      case "battery": {
        ...
        if (battery?.value === "low" || battery?.value === "critical") {
          await sb.from("ai_logs").insert({
            ...
            admin_id: "bouncie-webhook",
          }).catch((err) => console.warn("bouncie-webhook: battery log failed:", err.message));
        }
        break;
      }
    ...
    // Return 200 so Bouncie does not retry on our internal errors
