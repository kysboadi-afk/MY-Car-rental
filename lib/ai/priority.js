...
export const UNKNOWN_PRIORITY_ORDER = 3; // fallback when priority field is unrecognised

// Action status progression order (lower = earlier stage)
export const ACTION_STATUS_ORDER = { pending: 0, in_progress: 1, resolved: 2 };
...
export function sortByPriority(entries) {
  return [...entries].sort(
    (a, b) => (PRIORITY_ORDER[a.priority] ?? UNKNOWN_PRIORITY_ORDER) - (PRIORITY_ORDER[b.priority] ?? UNKNOWN_PRIORITY_ORDER)
  );
}
