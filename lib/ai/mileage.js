...
export function analyzeMileage(mileageData = [], recentTrips = []) {
  ...
  for (const row of mileageData) {
    ...
    const daysWithTrips    = new Set(vehicleTrips30d.map((t) => (t.trip_at || "").slice(0, 10))).size;
...
function buildMaintenanceAlert(name, milesSinceService) {
  ...
  if (milesSinceService >= OIL_CHANGE_MILES) {
    return `${name}: ${fmt(milesSinceService)} miles since last service — oil change due (every ${fmt(OIL_CHANGE_MILES)}–4,000 mi)`;
  }
