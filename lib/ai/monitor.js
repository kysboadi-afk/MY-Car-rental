...
import { analyzeMileage } from "./mileage.js";
import { analyzeFleetUsage } from "./usage.js";

// Bouncie sync is considered "recent" if last_synced_at is within the last 48 hours.
const BOUNCIE_ACTIVE_WINDOW_MS = 48 * 60 * 60 * 1000;

/**
 * Returns true if the vehicle has an assigned Bouncie device with a recent sync.
 * This is the source of truth for tracking activity — independent of booking/fleet status.
 *
 * @param {object} vehicle
 * @returns {boolean}
 */
export function isBouncieActive(vehicle) {
  if (!vehicle?.bouncie_device_id) return false;
  if (!vehicle?.last_synced_at)    return false;
  return (Date.now() - new Date(vehicle.last_synced_at).getTime()) <= BOUNCIE_ACTIVE_WINDOW_MS;
}
 ...
 * @param {Array}    [params.mileageData]      - optional vehicle mileage rows for maintenance alerts
 * @param {Array}    [params.recentTrips]      - optional trip_log rows for usage/idle analysis
 * @returns {string[]} Array of human-readable problem descriptions
 */
export function detectProblems({ allBookings, vehicles, revenueFromBooking, insights, mileageData, recentTrips }) {
  const problems = [];
  const now      = new Date();
  ...
  for (const [vehicleId, vehicle] of Object.entries(vehicles)) {
    // Skip truly decommissioned vehicles unless they are actively tracked by Bouncie.
    // rental_status ("rented", "reserved") is intentionally ignored here — a vehicle
    // being rented is still active and should be monitored.
    if (vehicle.status === "inactive" && !isBouncieActive(vehicle)) continue;
  ...
  // ── 8. Mileage-based alerts (maintenance due) ────────────────────────────
  if (mileageData && mileageData.length > 0) {
    const { alerts: mileageAlerts } = analyzeMileage(mileageData, recentTrips || []);
    for (const alert of mileageAlerts) {
      problems.push(alert);
    }
  }

  // ── 9. Usage-based alerts (real-time: miles_24h, miles_7d, idle, overuse) ─
  if (mileageData && mileageData.length > 0 && recentTrips && recentTrips.length > 0) {
    const { alerts: usageAlerts } = analyzeFleetUsage(mileageData, recentTrips);
