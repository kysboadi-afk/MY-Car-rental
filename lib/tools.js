...
export const TOOL_DEFINITIONS = [
  ...
  },
  {
    type: "function",
    function: {
      name: "run_system_health_fix",
      description:
        "Run backend system-health repair actions used by the Admin > System Health 'Fix Now' controls. " +
        "Use this when the admin asks to fix health issues, repair missed SMS, or repair availability sync. " +
        "Requires admin confirmation because it can send SMS and write database repairs.",
      parameters: {
        type: "object",
        properties: {
          target: {
            type: "string",
            enum: ["smsDeliveryHealth", "availabilitySyncHealth", "all"],
            description:
              '"smsDeliveryHealth" repairs missed active-rental SMS reminders, ' +
              '"availabilitySyncHealth" repairs blocked_dates availability rows, ' +
              '"all" runs both repairs (default).',
          },
          confirmed: {
            type: "boolean",
            description: "Must be true to execute after admin confirmation.",
  ...
  },
  {
    ...
    function: {
      name: "delete_booking",
      description:
        "Permanently delete a booking and all associated records (revenue, charges, SMS logs, extensions) from the system. " +
        "Only cancelled or pending bookings may be deleted. " +
        "Use get_bookings to confirm the booking_ref and status before calling this. " +
        "This action is irreversible. Requires admin confirmation before executing.",
      parameters: {
        type: "object",
        properties: {
          booking_id: {
            type: "string",
            description: 'The booking_ref of the booking to permanently delete (e.g. "bk-abc123").',
          },
          confirmed: {
            type: "boolean",
            description: "Must be true to execute after admin confirmation.",
  ...
  },
  {
    type: "function",
    function: {
      name: "backfill_stripe_cards",
      description:
        "Recover missing Stripe card references for all bookings in two phases: " +
        "Phase 1 restores stripe_customer_id and stripe_payment_method_id (main payment card) for bookings " +
        "where those fields were wiped by a past bug. " +
        "Phase 2 restores extension_stripe_customer_id and extension_stripe_payment_method_id (extension payment card) " +
        "for bookings that had a paid extension before those columns were added. " +
        "Always run with action='preview' first to see what would be changed before committing. " +
        "Use action='backfill' (requires confirmation) to apply the updates. " +
        "Safe to run multiple times — uses COALESCE semantics (never overwrites an existing value). " +
        "Use this when the admin asks to 'backfill cards', 'recover saved cards', 'save cards', or 'fix missing payment methods'.",
      parameters: {
        type: "object",
        properties: {
          action: {
            type: "string",
            enum: ["preview", "backfill"],
            description:
              '"preview" (default) — dry-run: shows which bookings would be patched and what the new values would be, without writing anything. ' +
              '"backfill" — applies the updates. Requires admin confirmation.',
          },
          confirmed: {
            type: "boolean",
            description: "Must be true when action is 'backfill'. Ask the admin to confirm before executing.",
          },
        },
        required: [],
      },
  ...
  },
  {
    ...
    function: {
      name: "defer_late_fee",
      description:
        "Flag a booking's late fee as 'pending_collection' so it is automatically " +
        "added to the renter's NEXT payment (rental extension or balance payment). " +
        "Use this when a late fee was assessed but cannot be charged off-session because " +
        "the renter has no saved card on file. " +
        "Always call get_bookings first to confirm the booking exists and check the current late_fee_status. " +
        "Requires admin confirmation before executing.",
      parameters: {
        type: "object",
        properties: {
          booking_id: {
            type: "string",
            description: 'booking_ref of the booking to update (e.g. "bk-abc123").',
          },
          amount: {
            type: "number",
            description:
              "Late fee amount in USD to collect on the renter's next payment (e.g. 25 or 35). " +
              "Required — this is the amount that will be added to their next extension total.",
          },
          confirmed: {
            type: "boolean",
            description: "Must be true after the admin has confirmed the action.",
          },
        },
